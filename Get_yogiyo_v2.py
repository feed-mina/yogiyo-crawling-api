...
def get_available_services(lat, lng, access_token):
    headers = BASE_HEADERS.copy()
    headers["Authorization"] = f"Bearer {access_token}"
    ...
    if response.status_code == 200:
        return response.json()
    print("[오류] 배달 가능 여부 조회 실패:", response.status_code, response.text)
    return None
def get_user_info(access_token):
    ...
    if response.status_code == 200:
        return response.json()
    print("[오류] 사용자 정보 조회 실패:", response.status_code, response.text)
    return None
