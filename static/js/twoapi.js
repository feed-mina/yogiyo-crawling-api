...
function initialize(products) {
  ...
  function showProduct(blob, product) {
   ...
   // console.log(product.thumbnail_url);
    const thumbnailsrc = product.logo_url;
    // console.log(product)
    ...
    section.setAttribute('id', product.id);
    ...
    text = 'http://127.0.0.1:5000/3?id=' + decodeURI(product.id) +'&deliveryfee=' +decodeURI(product.adjusted_delivery_fee);
    ...
    para3.textContent = `배달비 ${product.adjusted_delivery_fee}원`;
    const deliveryfee = window.sessionStorage.setItem('deliveryfee', para3.textContent);
    //console.log(product.adjusted_delivery_fee)
    
    para4.textContent = `  ⌚${product.estimated_delivery_time}`;
    para5.textContent = `${product.distance.toFixed(2)}km`;

    main.appendChild(hrefwrap_store);
    hrefwrap_store.appendChild(section);
