  ...
  function showProduct(json) {
    ...
    for(let i = 0; i<parse_Lode_json.length; i++ ){
      ...
      for(let v = 0; v<parse_Lode_json[i].items.length; v++){
        const reviewitemid = parse_Lode_json[i].items[v].id

    const itemstr = json.menu_items
if(json.menu_items != undefined){ 
    for (let m = 0; m < itemstr.length; m++) {
      let review_images_name = json.menu_items[m].name;
      let ids = json.menu_items[m].id;
      //    console.log(json.menu_items[m])
       if(ids == reviewitemid){
        console.log(ids);
         //    console.log(review_images_name);
        console.log(reviewitemid);

       section.appendChild(reviewdiv3);
       nameargs.push(review_images_name);

      //      console.log(nameargs)
       const nameset = new Set(nameargs);
        const namenewset = [...nameset];
        console.log(namenewset)
        reviewdiv3.textContent = `${namenewset}`;
       if(parse_Lode_json[i].items[v].original_image != undefined){
        let review_images_original = parse_Lode_json[i].items[v].original_image
        console.log(review_images_original)
        imgargs.push(review_images_original)
        const imgset = new Set(imgargs)
        const imgnewset = [...imgset];
        console.log(imgnewset);
        imgnewset.map(({src})=>menuphoto.src)

      //  menuphoto.setAttribute('id',parse_Lode_json[i].items[v].id)
    //    menuphoto.setAttribute('class','menuphoto')
    var imgs = new Array();
        for(var r = 0; r<imgnewset.length; r++){
          imgs[r] = new Image();
          imgs[r].src = imgnewset[r]
          console.log(imgnewset[r])

       menuphoto.setAttribute('src',imgs[r].src)
        }
       reviewdiv3.setAttribute('id',ids)
  }
  }
}
}
